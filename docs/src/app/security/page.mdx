...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Window create/focus/close operations and overlay WebView management</td>
    </tr>
    <tr>
...
zero-native provides built-in commands for windows (`zero-native.window.*`), overlay WebViews (`zero-native.overlay.*`), and dialogs (`zero-native.dialog.*`). These are controlled separately from app-defined commands via the `builtin_bridge` field in `RuntimeOptions`.

`js_window_api` exposes the JavaScript window and overlay WebView helpers, but it does not bypass security. Window commands (`zero-native.window.list`, `create`, `focus`, `close`) and overlay commands (`zero-native.overlay.create`, `setFrame`, `navigate`, `setZoom`, `close`) must come from an allowed origin and must have the `window` permission when runtime permissions are configured. Overlay commands can only target the window that sent the bridge message. Overlay URLs must also be allowed by `security.navigation.allowed_origins`, and overlay pages do not receive `window.zero` by default.

For broader control, use an explicit `builtin_bridge` policy. When you choose this path, list every built-in command your app calls:
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>Builtin bridge (windows and overlay WebViews)</td>
