...
- `browser` is a vanilla no-build shell that uses overlay WebViews for isolated page content.
...
Start with `hello`, then move to `webview` when you need native commands or WebView policy. Use `browser` when you want to see native overlay WebViews, and use a framework example when building a real frontend.
